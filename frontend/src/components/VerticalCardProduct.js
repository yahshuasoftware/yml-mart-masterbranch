...
const VerticalCardProduct = ({ category, heading }) => {
  ...
  // Handle right scroll
  const scrollRight = () => {
    const scrollAmount = 300; // You can tweak this amount based on your design
  ...
  // Handle left scroll
  const scrollLeft = () => {
    const scrollAmount = 300; // Adjust this based on your card width
    if (scrollElement.current) {
      scrollElement.current.scrollLeft -= scrollAmount;
      setScrollPosition(scrollElement.current.scrollLeft - scrollAmount); // Update the scroll position
    }
  ...
  return (
    <div className="container mx-auto px-10 my-6 relative">
      ...
      <button className='bg-white shadow-md rounded-full p-1 absolute left-2 top-1/2 text-lg hidden md:block' 
      onClick={scrollLeft}
      
      >
                    <FaAngleLeft />
        </button>
        <button className='bg-white shadow-md rounded-full p-1 absolute right-2   top-1/2 text-lg hidden md:block' onClick={scrollRight}>
                    <FaAngleRight />
         </button>
